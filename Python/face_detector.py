...
def detect_canny_edges_and_faces_from_webcamera():
    cap = cv.VideoCapture(0)
    cascade = cv.CascadeClassifier(
        "data/lbpcascade_frontalface_improved.xml")  # Загрузка обученного каскадного классификатора
    while True:
        ok, img = cap.read()
        if not ok:
            break
        ...
        # Вокруг найденного лица рисуем красный прямоугольник
